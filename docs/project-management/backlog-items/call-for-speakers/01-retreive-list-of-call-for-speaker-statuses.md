## Retrieve a List of Call for Speakers Statuses

As a Hermes user, I want to retrieve the list of call for speakers statuses. The list can be displayed either in the console or outputted to a JSON file.

#### Command Line Arguments

```bash
$ hermes callforspeakers statuses [--format {console|JSON}] [--output {callforspeakers-status.json}]
```
...
- When the user runs the command with no arguments, the CLI should display the list of all call for speakers statuses within the console.

- The list should be sorted by the call for speakers status sort order value.

- If there are no call for speakers status to display, the CLI should provide a message indicating that there are no configured call for speakers statuses.

- If the user include the "--format json" (case insensitive) option, the list of call for speakers status will be saved to a JSON file.

  - If the `--output` option is not specified, the call for speakers statuses will be saved to the `callforspeakers-statuses.json` file with the current directory.
